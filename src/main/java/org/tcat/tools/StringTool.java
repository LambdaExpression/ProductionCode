...
public class StringTool extends StringUtils {
     ...
     * @return
     */
    public static String initials(String content) {
        if (!StringTool.isEmptyByTrim(content)) {
            return Arrays.stream(content.split(" |_")).map(i -> {
                return firstLetterUppercase(i);
            }).collect(Collectors.joining());
        }
        return null;
    }
    ...
    /**
     * 转成首字母大写,除了第一个字母是小写（空格和下划线后的字母都是首字母）
     * <pre>
     *      abc def_gh   ->  abcDefGh
     * </pre>
     *
     ...
     * @return
     */
    public static String initialsExceptFirst(String content) {
        if (!StringTool.isEmptyByTrim(content)) {
            return firstLetterLowercase(Arrays.stream(content.split(" |_")).map(i -> {
                return firstLetterUppercase(i);
            }).collect(Collectors.joining()));
        }
        return null;
     ...
     * @return
     */
    public static String firstLetterUppercase(String content) {
        if (!StringTool.isEmptyByTrim(content)) {
            StringBuilder sb = new StringBuilder(content);
            sb.setCharAt(0, Character.toUpperCase(content.charAt(0)));
            return sb.toString();
        }
        return null;
    }
     ...
     * @return
     */
    public static String firstLetterLowercase(String content) {
        if (!StringTool.isEmptyByTrim(content)) {
            StringBuilder sb = new StringBuilder(content);
            sb.setCharAt(0, Character.toLowerCase(content.charAt(0)));
            return sb.toString();
        }
        return null;
    }
