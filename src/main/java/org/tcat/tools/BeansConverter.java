...
public class BeansConverter {
    ...
    public static Map<String, Object> beanToMap(Object bean, boolean ignorNull) {
        ...
        try {
            ...
            for (PropertyDescriptor descriptor : descriptors.values()) {
                ...
                if (ignorNull) {
                    if (result != null) {
                        returnMapPut(returnMap, propertyName, result, ignorNull);
                    }
                } else {
                    returnMapPut(returnMap, propertyName, result, ignorNull);
                }
    ...
    private static synchronized void returnMapPut(Map<String, Object> returnMap, String propertyName, Object result, boolean ignorNull) {
        if (result instanceof Collection) {
            Collection collection = (Collection) result;
            List<Map<String, Object>> returnMapColl = new ArrayList<>();
            collection.forEach(i -> {
                returnMapColl.add(beanToMap(i, ignorNull));
            });
            returnMap.put(propertyName,returnMapColl);
        } else {
